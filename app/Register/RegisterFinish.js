...
const RegisterSecond = () => {
    ...
    useEffect(() => {
        ...
        textFirstAnimX.value = withSpring(-width / 3.2, { damping: 50, stiffness: 100 });
        textSecondAnim.value = withSpring(-height / 4, { damping: 50, stiffness: 100 });
        textSecondAnimX.value = withSpring(-width / 9.5, { damping: 50, stiffness: 100 });
    }, [height]);
    //////////////////////////////////////////////////////////////
    ...
    const createAccount = async () => {
        try {
          ...
          // Kullanıcı kaydı başarılı olduğunda, AsyncStorage'a kayıt durumunu kaydedin
    ...
    return (
        <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'padding'}>
            <View style={styles.container}>
                ...
                {/* Text Animated */}
                <Animated.Text style={[styles.text, textFirstStyle, { color: 'white' }]}>Last</Animated.Text>
                <Animated.Text style={[styles.text, textSecondStyle, { color: 'rgba(255,0,0,0.8)',fontSize:40 }]}>Set Username</Animated.Text>
                {/* Input Container */}

                <View style={{ width: '100%', height: '50%', justifyContent: 'center', alignItems: 'center', marginVertical: 10 }}>
                    <Text style={{ color: 'rgba(255,255,255,0.9)', alignSelf: 'baseline' }}>Username</Text>
                    ...
                    <TouchableOpacity style={styles.button} onPress={createAccount}>
                        <Text style={{ color: 'white', fontWeight: '500', fontSize: 20, textAlign: 'center' }}>Finished</Text>
                    </TouchableOpacity>
                </View>
                {/* Footer */}
            </View>
